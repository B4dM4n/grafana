...
func (s *SocialGenericOAuth) UserInfo(ctx context.Context, client *http.Client, token *oauth2.Token) (*social.BasicUserInfo, error) {
	...
	err = s.postProcessUserInfo(ctx, client, userInfo, externalOrgs, externalOrgRoles)
	if err != nil {
		return nil, err
	}
...
func (s *SocialGenericOAuth) extractOrgRoles(externalOrgRoles *map[string]string, data *UserInfoJson) error {
